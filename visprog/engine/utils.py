...
class ProgramGenerator():
    def __init__(self,prompter,temperature=0.7,top_p=0.5,prob_agg='mean'):
        ...
        # self.opt_model = OPTForCausalLM.from_pretrained("facebook/opt-350m")
        # self.opt_tokenizer = AutoTokenizer.from_pretrained("facebook/opt-350m")
    ...
    def generate_llama(self,inputs):
        prompt = self.prompter(inputs)
        response = self.llama_model(prompt) # , max_tokens=0
    ...
    # def generate_opt(self,inputs):
    #     prompt = self.prompter(inputs)
    #     tok_inputs = self.opt_tokenizer(prompt, return_tensors="pt")
    #     generate_ids = self.opt_model.generate(tok_inputs.input_ids, max_length=2500)
    #     prog = self.opt_tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
