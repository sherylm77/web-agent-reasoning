...
class ActionInterpreter():
    ...
    def html(self,id,answer,output_var):
        step_name = html_step_name(self.step_name)
        answer = html_output(answer)
        output_var = html_var_name(output_var)
        id_arg = html_arg_name('id')
        return f"""<div>{output_var}={step_name}({id_arg}='{id}')={answer}</div>"""

    def execute(self,prog_step,inspect=False):
        id_var,output_var = self.parse(prog_step)
        id = prog_step.state[id_var]
        answer = self.predict(id)
        prog_step.state[output_var] = answer
        if inspect:
            html_str = self.html(id, answer, output_var)
            return answer, html_str

        return answer

class GetIDInterpreter():
    step_name = 'ID'
    ...
    def parse(self,prog_step):
        ...
        img_var = args['image']
        output_var = parse_result['output_var']
        tree = args['accessibility_tree']
        assert(step_name==self.step_name)
        return img_var,tree,output_var

    def predict(self,img,accessibility_tree):
        # img = img.resize(img.size*2, Image.Resampling.LANCZOS)
        img_filename = "../assets/temp.png"
        img.save(img_filename)
        elements = partition_image(img_filename, strategy='ocr_only')

        text = []

        for element in elements:
            text += (element.text.lower().split())
            print(element.text)
        ...
        for line in accessibility_tree.split("\t"):
            line = re.sub("/[^a-zA-Z0-9 ]/g", "", line.lower())
            print("tree", line)
            for item in text:
                print("text", item)
                if item in line:
    ...
    def html(self,img,tree,answer,output_var):
        step_name = html_step_name(self.step_name)
        img_str = html_embed_image(img)
        answer = html_output(answer)
        output_var = html_var_name(output_var)
        image_arg = html_arg_name('image')
        tree_arg = html_arg_name('tree')
        return f"""<div>{output_var}={step_name}({image_arg}={img_str},&nbsp;{tree_arg}='{tree}')={answer}</div>"""

    def execute(self,prog_step,inspect=False):
        img_var,tree,output_var = self.parse(prog_step)
        img = prog_step.state[img_var]
        answer = self.predict(img,tree)
        prog_step.state[output_var] = answer
        if inspect:
            html_str = self.html(img, tree, answer, output_var)
            return answer, html_str
...
class ContentInterpreter():
    ...
    def __init__(self):
        print(f'Registering {self.step_name} step')
        self.llama_model = Llama(model_path="..\\llama-2-7b-chat.ggmlv3.q8_0.bin", n_ctx=2048)
    ...
    def parse(self,prog_step):
        ...
        tree = args['accessibility_tree']
        assert(step_name==self.step_name)
        return content,tree,output_var

    def predict(self,content,accessibility_tree):
        res = []
        lines = accessibility_tree.split("\t")
        for i in range(len(lines)):
            line = lines[i]
            line = re.sub("/[^a-zA-Z0-9 ]/g", "", line.lower())
            prompt = "Does this line mention {}? Answer with yes or no.".format(content)
            response = self.llama_model(prompt)
            response = response["choices"][0]["text"].lstrip('\n').rstrip('\n')
            if "yes" in response:
                res.append(line[i+2].split()[1:]) # remove the StaticText
            
        return res

    def html(self,content,tree,answer,output_var):
        step_name = html_step_name(self.step_name)
        answer = html_output(answer)
        output_var = html_var_name(output_var)
        content_arg = html_arg_name('content')
        tree_arg = html_arg_name('tree')
        return f"""<div>{output_var}={step_name}({content_arg}={content},&nbsp;{tree_arg}='{tree}')={answer}</div>"""

    def execute(self,prog_step,inspect=False):
        content,tree,output_var = self.parse(prog_step)
        answer = self.predict(content,tree)
        prog_step.state[output_var] = answer
        if inspect:
            html_str = self.html(img, content, tree, answer, output_var)
            return answer, html_str

        return answer


class WebLocInterpreter():
    step_name = 'WEBLOC'
    ...
    def predict(self,img,obj_name):
        img_filename = "../assets/"+obj_name+".png"
        img.save(img_filename)
        elements = partition_image(img_filename)
        ...
        for element in elements:
            ...
            # print(element.text)
            if obj_name in element.text.lower():
                ...
                scores.append(metadata['detection_class_prob'])
...
def register_step_interpreters(dataset='nlvr'):
    if dataset=='nlvr':
        ...
        return dict(
            ...
            EVAL=EvalInterpreter(),
            RESULT=ResultInterpreter(),
            ACT=ActionInterpreter(),
            WEBLOC=WebLocInterpreter(),
            ID=GetIDInterpreter(),
            CONT=ContentInterpreter()
        )
